...
# Load data
link = pd.read_csv(r"C:/Users/ajeet/linkedin_data 2023-03-31_final.csv")
print(link.columns)
st.write(link.head())
...
# Sidebar
chart_selection = st.sidebar.selectbox("Select a chart", ("Job Name", "Company", "City", "Involvement", "Industry"))

# Show selected chart
if chart_selection == "Job Name":
    link1['Job_Name'].value_counts()[:5].plot(kind='pie', autopct='%1.1f%%', startangle=180, shadow=True, explode=[0.1, 0, 0, 0, 0])
    fig = plt.gcf()
    fig.set_size_inches(7, 7)
    st.pyplot(fig)

elif chart_selection == "Company":
    link1['Company'].value_counts()[:5].plot(kind='pie', autopct='%1.1f%%', startangle=180, shadow=True, explode=[0.2, 0, 0, 0, 0], figsize=(8, 8))
    fig = plt.gcf()
    st.pyplot(fig)

elif chart_selection == "City":
    link1['City'].value_counts()[:5].plot(kind='pie', autopct='%1.1f%%', startangle=180, shadow=True, explode=[0.1, 0, 0, 0, 0])
    fig = plt.gcf()
    fig.set_size_inches(7, 7)
    st.pyplot(fig)

elif chart_selection == "Involvement":
    link1['Invovlement'].value_counts()[:5].plot(kind='pie', autopct='%1.1f%%', startangle=180, shadow=True, explode=[0.1, 0, 0, 0, 0])
    fig = plt.gcf()
    fig.set_size_inches(7, 7)
    st.pyplot(fig)

elif chart_selection == "Industry":
